...
# Load marks from TXT and merge with students data
marks_dict = {}
with open("marks.txt") as txtfile:
    for line in txtfile:
...
for student in students:
    student["marks"] = marks_dict.get(student["name"], "N/A")
...
marks_label = tk.Label(info_frame, text="Marks: ", font=("Arial", 12), bg="#fff3e0")

name_label.pack(anchor="w", pady=5)
age_label.pack(anchor="w", pady=5)
major_label.pack(anchor="w", pady=5)
marks_label.pack(anchor="w", pady=5)
...
def show_info(student):
    ...
    marks_label.config(text=f"Marks: {student['marks']}")
