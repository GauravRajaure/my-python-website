...
# Set up button to show image and copy itself:
    imgButton = ttk.Button(window,image = imgDefault, style="TButton",
    command = lambda : copyImg(window,imgDefault))

    imgButton.grid(row=4, column=0, sticky=tk.NE)
    imgButton.image = imgDefault

# function to handle button copy:
def copyImg(window, img):
    imgButton = ttk.Button(window,image = img, style="TButton",
    command = lambda : copyImg(window,imgButton))
